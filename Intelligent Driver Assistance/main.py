...
class MainWindow(QMainWindow):
    ...
    def tick_callback(self):
        time.sleep(0.2)
        # 添加帧有效性校验逻辑
        if self.is_valid_frame(self.im0):
            # 调用显示函数前再次验证帧数据
            self.show_image_on_label(self.im0, self.label_result, 951, 571)
        self.show_info_on_text(self.info)
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if view_img:
                    ...
                    # time.sleep(0.1)
                    if self.p == "person":
                        self.info = "检测到行人！请注意提前减速慢行，保持安全车距，随时准备停车避让，避免发生碰撞事故。"
                    elif self.p == "bicycle":
                        self.info = "检测到自行车！请提前减速，保持足够的横向距离，避免因距离过近引发剐蹭事故。"
                    elif self.p == "car":
                        self.info = "前方检测到车辆！请保持安全车距，注意观察前车动态，避免急刹车和频繁变道，防止追尾事故发生。"
                    elif self.p == "motorcycle":
                        self.info = "注意摩托车！摩托车速度较快且车身较窄，容易在视线盲区出现，务必提高警惕，保持安全距离，切勿随意变道挤占其行驶空间。"
                    elif self.p == "bus":
                        self.info = "检测到公交车！公交车体型庞大，停靠站点频繁，请注意观察其转向灯和刹车灯，保持足够车距，避免因视线遮挡而发生危险。"
                    elif self.p == "truck":
                        self.info = "检测到大型车辆！卡车车身长、盲区范围广，请勿长时间在其盲区行驶，超车时要果断迅速，保持安全距离，谨防其突然转向或刹车。"
                    elif self.p == "traffic light":
                        self.info = "注意交通信号灯！提前观察信号灯变化，减速慢行，严格遵守交通规则，避免因抢行闯红灯引发交通事故。"
                    elif self.p == "stop sign":
                        self.info = "前方有停车标志！请立即减速停车，仔细观察四周交通情况，确认安全后再继续行驶，切勿忽视停车标志强行通过。"
                    else:
                        self.info = ""
    ...
    def stop_detection(self):
        ...
        self.info = ""
        # 清空显示的图像
        self.im0 = cv2.imread('source/bg.png', cv2.IMREAD_COLOR)
